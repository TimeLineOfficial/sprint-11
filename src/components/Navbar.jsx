    ...
    <>
      <header className="sticky top-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 shadow-sm transition-colors">
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            ...
            <Link to="/" className="flex items-center space-x-2.5">
              ...
              <div className="flex flex-col">
                <span className="font-extrabold text-sm sm:text-base tracking-tight text-slate-900 dark:text-white leading-none">
                  FINPULSE<span className="text-emerald-600 dark:text-emerald-400">AI</span>
                </span>
                <span className="text-[8px] sm:text-[9px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mt-0.5 truncate max-w-[170px] sm:max-w-none">
              ...
              return (
                <Link
                  ...
                  className={`px-3.5 py-2 rounded-xl transition-all ${
                    isActive
                      ? 'bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-extrabold border border-emerald-500/40 shadow-sm'
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 text-xs">
            <div className="px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border border-emerald-500/30 text-[10px] font-extrabold hidden sm:flex items-center gap-1.5">
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          />
          <div className="relative w-4/5 max-w-xs bg-white dark:bg-slate-900 h-full shadow-2xl flex flex-col z-10 border-r border-slate-200 dark:border-slate-800 transition-colors">
            <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-extrabold text-sm text-slate-900 dark:text-white">FINPULSE MENU</span>
              </div>
              <button
                onClick={() => setIsMobileMenuOpen(false)}
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-800 space-y-3">
              ...
              <div className="text-[10px] text-slate-400 text-center flex items-center justify-center gap-1">
                <ShieldCheck className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> FinPulse SEC &amp; CFTC Compliant Core
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-slate-900/95 backdrop-blur-lg border-t border-slate-200 dark:border-slate-800 py-1.5 px-2 flex items-center justify-around">
        {navLinks.map((link) => {
          const IconComp = link.icon;
          const isActive = location.pathname === link.path;
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center justify-center py-1 px-2.5 rounded-xl transition-all ${
                isActive ? 'text-emerald-600 dark:text-emerald-400 font-extrabold' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'
              }`}
            >
              <IconComp className={`w-5 h-5 ${isActive ? 'scale-110 text-emerald-600 dark:text-emerald-400' : ''} transition-transform`} />
