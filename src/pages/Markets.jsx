  ...
  return (
    <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-6">
      ...
      <div className="bg-slate-800 p-4 sm:p-5 rounded-2xl border border-slate-700 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4 text-xs">
        <div>
          <div className="text-[10px] sm:text-[11px] font-bold text-emerald-400 uppercase tracking-wider mb-1 flex items-center gap-1.5">
            <ShieldCheck className="w-3.5 h-3.5 text-emerald-400" /> Real-Time Institutional Liquidity Feeds
          </div>
      ...
      <div className="bg-slate-800 border border-slate-700 rounded-2xl overflow-hidden shadow-xl text-xs">
        <div className="hidden sm:grid grid-cols-12 gap-4 p-4 border-b border-slate-700 font-extrabold text-slate-400 uppercase text-[10px]">
        ...
        <div className="divide-y divide-slate-700/60">
          {filteredAssets.map((asset) => (
            <div key={asset.id} className="p-4 sm:grid sm:grid-cols-12 sm:gap-4 sm:items-center space-y-2 sm:space-y-0 hover:bg-slate-700/50 transition-colors">
              <div className="sm:col-span-4 flex items-center justify-between sm:justify-start space-x-3">
                <div className="w-10 h-10 rounded-xl bg-slate-900 border border-slate-700 font-extrabold text-emerald-400 flex items-center justify-center text-xs">
                  {asset.id}
                </div>
                <div>
                  <div className="font-extrabold text-white text-sm">{asset.name}</div>
                  <div className="text-[10px] text-slate-400">{asset.type} • Vol: ${asset.volume24h}</div>
                </div>
              </div>

              <div className="sm:col-span-3 flex justify-between sm:justify-end items-center font-mono text-sm font-extrabold text-white">
                <span className="sm:hidden text-slate-400 text-xs font-normal">Price:</span>
              ...
              <div className="sm:col-span-3 flex justify-between sm:justify-end items-center">
                <span className="sm:hidden text-slate-400 text-xs">24h Change:</span>
