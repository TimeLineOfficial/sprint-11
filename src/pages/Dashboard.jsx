      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          {recentOrders.length > 0 && (
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 sm:p-5 space-y-3 shadow-sm text-xs">
              <div className="font-extrabold text-sm text-white border-b border-slate-700 pb-2.5 flex items-center justify-between">
                <span>Recent Algorithmic Execution Log</span>
                <span className="text-[10px] text-emerald-400 font-mono">{recentOrders.length} Orders</span>
              </div>
              <div className="space-y-2">
                {recentOrders.map((ord) => (
                  <div key={ord.id} className="flex items-center justify-between p-3 rounded-xl bg-slate-900 border border-slate-700/80">
                    <div>
                      ...
                      <div className="text-[10px] text-slate-400">{ord.timestamp} • ID: {ord.id}</div>
                    </div>
                    <div className="text-right">
                      <div className="font-mono font-bold text-white">${ord.total.toFixed(2)}</div>
                      <div className="text-[10px] text-slate-400">@ ${ord.price.toFixed(2)}</div>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="lg:col-span-6 space-y-6">
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 sm:p-5 space-y-3 shadow-sm text-xs">
            <div className="flex items-center justify-between border-b border-slate-700 pb-2.5">
              <h3 className="font-extrabold text-sm text-white flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-emerald-400" /> Live Market Ticker (24h)
              </h3>
              <Link to="/markets" className="text-[11px] font-bold text-emerald-400 hover:underline">
                View All →
              </Link>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {ASSETS.map((asset) => (
                <div key={asset.id} className="p-3.5 rounded-xl bg-slate-900 border border-slate-700/80 space-y-1.5 hover:border-emerald-500/50 transition-all">
                  ...
                  <div className="text-base font-extrabold text-white font-mono">${asset.price.toLocaleString()}</div>
                  <div className="text-[10px] text-slate-400">Vol: ${asset.volume24h}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-4 sm:p-5 space-y-3 shadow-sm text-xs">
            <h3 className="font-extrabold text-sm text-white border-b border-slate-700 pb-2.5 flex items-center gap-2">
              <PieChart className="w-4 h-4 text-purple-400" /> Portfolio Asset Allocation
            </h3>
